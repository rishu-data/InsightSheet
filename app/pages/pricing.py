...
_STATUS_WRAP = {
    "good": "flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-green-200 bg-green-50 p-6 w-full",
    "info": "flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-6 w-full",
    "warn": "flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-yellow-200 bg-yellow-50 p-6 w-full",
    "neutral": "flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm w-full",
}
...
def _status_card() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.icon("credit-card", class_name="h-5 w-5 text-indigo-600"),
                class_name="flex items-center justify-center h-11 w-11 rounded-xl bg-white shrink-0 border border-gray-200",
            ),
            rx.el.div(
                rx.el.div(
                    rx.el.h2(
                        SubscriptionState.status_label,
                    ...
                    rx.cond(
                        ...
                        rx.el.span(
                            ...
                            "Pro Active",
                            class_name="flex items-center gap-1.5 w-fit rounded-full bg-indigo-600 px-3 py-1 text-xs font-semibold text-white",
                        ),
                    ),
        ...
        rx.el.div(
            rx.cond(
                ...
                rx.el.button(
                    ...
                    class_name="flex items-center gap-2 w-fit rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-60 transition-colors",
                ),
            ),
            rx.cond(
                ...
                rx.el.button(
                    rx.cond(
                        ...
                        rx.el.div(
                            class_name="h-4 w-4 rounded-full border-2 border-gray-400/40 border-t-gray-700 animate-spin"
                        ),
                    ...
                    class_name="flex items-center gap-2 w-fit rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-60 transition-colors",
                ),
                rx.el.a(
        ...
        class_name=rx.match(
            SubscriptionState.status_tone,
            ("good", _STATUS_WRAP["good"]),
            ("info", _STATUS_WRAP["info"]),
            ("warn", _STATUS_WRAP["warn"]),
            _STATUS_WRAP["neutral"],
        ),
    )
    ...
    return rx.el.div(
        ...
        rx.cond(
            ...
            rx.el.div(
                ...
                rx.el.span("Pro Active"),
                class_name="flex items-center justify-center gap-2 w-full rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-600 mt-6",
            ),
            rx.el.button(
                rx.cond(
