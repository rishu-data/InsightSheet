...
def test_active_requires_verified_owner() -> None:
    for name in (
        "subscription.activated",
        "subscription.charged",
        "payment.captured",
    ):
        # Verified ownership activates as before.
        assert (
            verified_target_status(name, None, True)
            is SubscriptionStatus.ACTIVE
        )
        # Ambiguous ownership never reaches ACTIVE.
        assert (
            verified_target_status(name, None, False)
            is SubscriptionStatus.PENDING
        )
        assert (
            verified_target_status(name, SubscriptionStatus.FREE, False)
            is SubscriptionStatus.PENDING
        )
        assert (
            verified_target_status(name, SubscriptionStatus.CANCELLED, False)
            is None
        )
    # Non-activating events are unaffected by ownership verification.
    assert (
        verified_target_status("payment.failed", None, False)
        is SubscriptionStatus.PAYMENT_FAILED
    )
    assert (
        verified_target_status("subscription.cancelled", None, False)
        is SubscriptionStatus.CANCELLED
    )
    assert verified_target_status("order.paid", None, True) is None
...
def main() -> None:
    ...
    test_active_requires_verified_owner()
