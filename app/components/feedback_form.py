...
def _sign_in_prompt() -> rx.Component:
    return rx.cond(
        AuthState.is_authenticated,
        rx.fragment(),
        rx.el.div(
            rx.icon(
                "log-in", class_name="h-4 w-4 text-blue-600 shrink-0 mt-0.5"
            ),
            rx.el.div(
                rx.el.p(
                    "Sign in to save your feedback",
                    class_name="text-sm font-semibold text-gray-900",
                ),
                rx.el.p(
                    "Feedback is stored against your account so your history is here next time. We don't store anonymous feedback.",
            ...
            rx.el.a(
                "Sign in",
                href="/login",
                class_name="w-fit shrink-0 rounded-xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-700 transition-colors",
            ),
            class_name="flex flex-wrap items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3",
        ),
    )
        ...
        rx.el.div(
            ...
            rx.cond(
                ...
                rx.el.span(
                    ...
                    class_name="flex items-center gap-1.5 w-fit shrink-0 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700",
                ),
                rx.el.span(
                    rx.icon("lock", class_name="h-3.5 w-3.5"),
        ...
        rx.el.div(
            rx.el.div(
                ...
                rx.el.p(
                    rx.cond(
                        AuthState.is_authenticated,
                        "Saved to your account — only you can see the feedback you've sent.",
                        "Sign in to see the feedback saved to your account.",
                    ),
                    class_name="text-sm font-medium text-gray-500",
                ),
                class_name="min-w-0 flex-1",
            ...
            rx.el.div(
                ...
                rx.el.p(
                    rx.cond(
                        ...
                        "Sign in to save feedback and see your history here.",
                    ),
...
def feedback_info_card() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.p(
            "Feedback is saved securely to your account and is never attached to your uploaded "
            "spreadsheet. Only you can see your own history, and it helps us decide which part "
            "of InsightSheet to improve next.",
